    ...
    public class Worker implements Runnable {
        ...
        private boolean parseCommand (String s) {
            ...
            if        (split[0].equals("MOVE")) {
                ...
                if(c.uid == 0 && c.current == null) {
                    // Probably should do some database lookup here
                    // to get info about the person
                    try {
                        c.uid = Integer.parseInt(split[1]);
                        c.current = new Coordinate(ServerAPI.WORLD_WIDTH/2, 
                                                   ServerAPI.WORLD_HEIGHT/2);
                        return true;
                    } catch (Exception e) {}
                } else {
                    return false;
                    // hes already logged in?
                }
